/*
  ==============================================================================
    ...
    knobs; Lime is seven — four over three, the three centred, so the knob block reads
    as an inverted trapezoid — and the freed space goes to the plot of the applied gain
    curve over the input spectrum, which is the most informative thing the plugin can
    show: boost rides above the zero line wherever the signal is quiet, cut mirrors it
    below in down mode.
    ...
    plugin's own, in the order the signal meets it: input, the blend against dry, output.
    Every switch lives in one column to the right of both rows, so the eye finds them in
...
private:
    ...
    // Four over three, the three centred — an inverted trapezoid. The top row is the
    // pass trims; the bottom is the plugin's own: the masters that exist because a
    // plugin, unlike a module in a console, does not sit at a known level.
    Trim recIn, recOut, playIn, playOut;
    Trim inputGain, mix, outputGain;
